...
const ContactUsModal: React.FC<ContactUsModalProps> = ({
    ...
    return (
        <div className="fixed inset-0 flex  items-center justify-center bg-gray-800 bg-opacity-50 z-50 ">
            <div className="bg-site-main flex flex-col justify-center items-center  py-4 px-6 md:px-10 rounded-md w-[80%] md:w-auto">
                {isSent && <MdOutlineCheckCircle size={150} />}
                {(isNotSent || !hasConcern) && (
                    <MdOutlineErrorOutline size={150} />
                )}
                ...
                <button
                    className="border-2  px-10 py-2 rounded-md md:text-base text-xs hover:text-site-main hover:bg-site-secondary transition-colors duration-300"
                    onClick={onClose}
                >
                    Close
                </button>
